...
public class ProductController {
	...
	public ResponseEntity<Void> ajouterProduit(@RequestBody Product product) {
		...
		if (product.getPrix() < 1) throw new ProduitGratuitException("Vous ne pouvez pas ajouter un produit dont le prix de vente est égal à 0");
	...
	public MappingJacksonValue calculerMargeProduit() {
		...
		// On utilise un dictionnaire (clé, valeur) pour faciliter le formatage des
		// données
		Map<Product, Integer> dico = new HashMap<Product, Integer>();

		for (Product produit : listProduits) {

			int marge = produit.getPrix() - produit.getPrixAchat();
			dico.put(produit, marge);

		}

		MappingJacksonValue listProduitsMarge = new MappingJacksonValue(dico);

		// return produitMarge;
		return listProduitsMarge;
	}
